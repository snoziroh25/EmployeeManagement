...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/login").permitAll().antMatchers("/signup").permitAll()
				.antMatchers("/styleIndex.css").permitAll().antMatchers("/styleLogin.css").permitAll()
				.antMatchers("/styleSignUp.css").permitAll().antMatchers("/index/**").hasAuthority("ADMIN").anyRequest()
